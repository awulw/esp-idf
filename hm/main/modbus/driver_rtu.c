...
static modbus_err_t rtu_data_to_frame(uint8_t *data, uint8_t data_len, modbus_frame_t *frame)
		{
			if (frame->data_size <data_len - 2) return MODBUS_ERR_DATA_LEN;
			uint16_t crc = crc16(data, data_len);
			memcpy(frame->data, data, data_len);
			frame->data[data_len] = crc;
			frame->data[data_len + 1] = crc >> 8;
			frame->data_len = data_len + 2;
			return MODBUS_OK;
		}
static modbus_err_t rtu_frame_to_data(modbus_frame_t *frame, uint8_t *data, uint8_t *data_len)
		{
			uint16_t crc = crc16(frame->data, frame->data_len - 2);
