...
modbus_core_t *modbus_core_create(modbus_type_t type)
{
	...
	if (type == MODBUS_ASCII)
	{
		...
		bus = bus_create(rs485, serial_get_driver(rs485), &ascii_driver);
	}
	//else if (type == MODBUS_RTU)
	else
	{
		rs485= serial_create(0, 115200, NULL, 1024);
		bus= bus_create(rs485, serial_get_driver(rs485), &rtu_driver);
...
void modbus_core_task(void *parm)
{
	...
	while (1)
	{
		...
		{
			msg.status = bus_transaction(core->bus , msg.data, msg.data_len, msg.data, &(msg.data_len), 2000);
